...
void Node::test_all_collisions(std::vector<RigidBodiesDuo>* rigid_bodies_list, std::vector<std::shared_ptr<Node>> ancestors) {
    ...
    for(int i = 0; i < ancestors_size; i++) {
        std::vector<std::shared_ptr<RigidBody>> ancestor_object_list = ancestors[i]->get_object_list();
        ...
        for(int j = 0; j < ancestor_size; j++) {
            for(int k = 0; k < this_size; k++) {
                if(ancestor_object_list[j] != this->object_list[k]) {
                    RigidBodiesDuo rb_duo;
                    rb_duo.rigid_body = ancestor_object_list[j];
                    rb_duo.other_rigid_body = this->object_list[k];
    ...
    for(int i = 0; i < this_size; i++) {
        for(int j = this_size - 1; j > i; j--) {
            RigidBodiesDuo rb_duo;
            rb_duo.rigid_body = this->object_list[i];
            rb_duo.other_rigid_body = this->object_list[j];
            rigid_bodies_list->push_back(rb_duo);
        }
    }
